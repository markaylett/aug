      ...
      lib_(path)
{
    ...
    const struct augmod_control* ptr(initfn(name_.c_str(), &host));
    if (!ptr)
        throw error(__FILE__, __LINE__, EMODCALL, "augmod_init() failed");
    setdefaults(control_, *ptr, teardown);
}
...
module::start(augmod_session& session) const AUG_NOTHROW
{
    ...
    return AUGMOD_OK == control_.start_(&session);
}
              ...
              size_t size) const AUG_NOTHROW
{
    ...
    control_.event_(from, type, user, size);
}
                 ...
                 unsigned short port) const AUG_NOTHROW
{
    ...
    return AUGMOD_OK == control_.accepted_(&sock, addr, port);
}
                  ...
                  unsigned short port) const AUG_NOTHROW
{
    AUG_DEBUG2("connected(): id=[%d], addr=[%s], port=[%u]",
               sock.id_, addr, (unsigned)port);
    control_.connected_(&sock, addr, port);
}
                 ...
                 const char* issuer) const AUG_NOTHROW
{
    ...
    return AUGMOD_OK == control_.authcert_(&sock, subject, issuer);
}
