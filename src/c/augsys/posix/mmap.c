...
{
    /* EXCEPT: destroymmap_B_ -> munmap; */
    /* EXCEPT: munmap -> EAGAIN; */
    if (impl->mmap_.addr_
                 ...
                 int flags)
{
    ...
    if (createmmap_B_(impl, offset, len) < 0) {
        aug_freemem(mpool, impl);
        return NULL;
    }
    aug_retain(mpool);
...
aug_remmap_BIN(struct aug_mmap* mm, size_t offset, size_t len)
{
    ...
    return createmmap_B_(impl, offset, len);
}
