...
/* at Fri Feb 26 07:10:57 +0000 2010 */
...
namespace aug {
    ...
    {
        return verify(this_.get()->vtbl_->read_(this_.get(), buf, size));
    }

    inline aug_rsize
    readv(aug::obref<aug_stream> this_, const struct iovec* iov, int size)
    {
        return verify(this_.get()->vtbl_->readv_(this_.get(), iov, size));
    }

    inline aug_rsize
    write(aug::obref<aug_stream> this_, const void* buf, size_t size)
    {
        return verify(this_.get()->vtbl_->write_(this_.get(), buf, size));
    }

    inline aug_rsize
    writev(aug::obref<aug_stream> this_, const struct iovec* iov, int size)
    {
        return verify(this_.get()->vtbl_->writev_(this_.get(), iov, size));
    }
