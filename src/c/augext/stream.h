...
/* at Thu Sep 18 06:59:20 GMT Daylight Time 2008 */
...
namespace aug {
    ...
    {
        return verify(this_.get()->vtbl_->shutdown_(this_.get()), this_);
    }

    inline aug_rsize
    read(aug::obref<aug_stream> this_, void* buf, size_t size)
    {
        return verify(this_.get()->vtbl_->read_(this_.get(), buf, size), this_);
    }

    inline aug_rsize
    readv(aug::obref<aug_stream> this_, const struct iovec* iov, int size)
    {
        return verify(this_.get()->vtbl_->readv_(this_.get(), iov, size), this_);
    }

    inline aug_rsize
    write(aug::obref<aug_stream> this_, const void* buf, size_t size)
    {
        return verify(this_.get()->vtbl_->write_(this_.get(), buf, size), this_);
    }

    inline aug_rsize
    writev(aug::obref<aug_stream> this_, const struct iovec* iov, int size)
    {
        return verify(this_.get()->vtbl_->writev_(this_.get(), iov, size), this_);
    }
