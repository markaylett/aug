...
/* at Wed Feb 24 21:08:09 +0000 2010 */
...
namespace aug {
    ...
    {
        verify(this_.get()->vtbl_->shutdown_(this_.get()));
    }

    inline size_t
    read(aug::obref<aug_stream> this_, void* buf, size_t size)
    {
        return static_cast<size_t>(AUG_RESULT(verify(this_.get()->vtbl_->read_(this_.get(), buf, size))));
    }

    inline size_t
    readv(aug::obref<aug_stream> this_, const struct iovec* iov, int size)
    {
        return static_cast<size_t>(AUG_RESULT(verify(this_.get()->vtbl_->readv_(this_.get(), iov, size))));
    }

    inline size_t
    write(aug::obref<aug_stream> this_, const void* buf, size_t size)
    {
        return static_cast<size_t>(AUG_RESULT(verify(this_.get()->vtbl_->write_(this_.get(), buf, size))));
    }

    inline size_t
    writev(aug::obref<aug_stream> this_, const struct iovec* iov, int size)
    {
        return static_cast<size_t>(AUG_RESULT(verify(this_.get()->vtbl_->writev_(this_.get(), iov, size))));
    }
