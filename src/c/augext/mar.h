...
/* at Fri Nov 14 07:00:00 GMT Standard Time 2008 */
...
namespace aug {
    ...
    template <typename T>
    class basic_marpool {
        marpool<basic_marpool<T> > marpool_;
        T impl_;
        int refs_;
        explicit
        basic_marpool(const T& impl)
            : impl_(impl),
              refs_(1)
        {
    ...
    public:
        ...
        {
            basic_marpool* ptr(new basic_marpool(impl));
            return aug::object_attach<aug_marpool>(ptr->marpool_);
        }
    };

    template <typename T>
    class scoped_marpool {
        marpool<scoped_marpool<T> > marpool_;
        T impl_;
    public:
        explicit
        scoped_marpool(const T& impl = T())
            : impl_(impl)
        {
            this->marpool_.reset(this);
        }
