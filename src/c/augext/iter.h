/* -*- c++ -*- */
/* Automatically generated by augidl */
/* at Fri Mar 27 07:12:27 +0000 2009 */

#ifndef AUGEXT_ITER_H
#define AUGEXT_ITER_H

#include "augabi.h"

#if defined(__cplusplus)

#include "augabipp.hpp"

namespace aug {
    template <typename T>
    struct object_traits;
}

#endif /* __cplusplus */

/* Includes. */

#include "augtypes.h"

/* Interface declarations. */

AUG_INTERFACE(aug_iter);

/* Interface definitions. */

struct aug_var;

/**
 * @defgroup aug_iter aug_iter
 *
 * @ingroup Object
 *
 * @{
 */

struct aug_itervtbl {
    AUG_VTBL(aug_iter);
    aug_bool (*islast_)(aug_iter*);
    void (*next_)(aug_iter*);
    aug_result (*getkey_)(aug_iter*, struct aug_var*);
    aug_result (*getvalue_)(aug_iter*, struct aug_var*);
};

#define aug_isiterlast(this_) \
    (this_)->vtbl_->islast_(this_)

#define aug_nextiter(this_) \
    (this_)->vtbl_->next_(this_)

#define aug_getiterkey(this_, var) \
    (this_)->vtbl_->getkey_(this_, var)

#define aug_getitervalue(this_, var) \
    (this_)->vtbl_->getvalue_(this_, var)

/** @} */

#if defined(__cplusplus)
namespace aug {
    template <>
    struct object_traits<aug_iter> {
        typedef aug_itervtbl vtbl;
        static const char*
        id() AUG_NOTHROW
        {
            return aug_iterid;
        }
    };
}

namespace aug {

    template <typename T>
    class iter {

        aug_iter iter_;

        iter(const iter&);

        iter&
        operator =(const iter&);

        static void*
        cast_(aug_iter* this_, const char* id) AUG_NOTHROW
        {
            return aug::retget(static_cast<T*>(this_->impl_)->cast_(id));
        }
        static void
        retain_(aug_iter* this_) AUG_NOTHROW
        {
            static_cast<T*>(this_->impl_)->retain_();
        }
        static void
        release_(aug_iter* this_) AUG_NOTHROW
        {
            static_cast<T*>(this_->impl_)->release_();
        }
        static aug_bool
        islast_(aug_iter* this_) AUG_NOTHROW
        {
            return static_cast<T*>(this_->impl_)->isiterlast_();
        }
        static void
        next_(aug_iter* this_) AUG_NOTHROW
        {
            static_cast<T*>(this_->impl_)->nextiter_();
        }
        static aug_result
        getkey_(aug_iter* this_, struct aug_var* var) AUG_NOTHROW
        {
            return static_cast<T*>(this_->impl_)->getiterkey_(*var);
        }
        static aug_result
        getvalue_(aug_iter* this_, struct aug_var* var) AUG_NOTHROW
        {
            return static_cast<T*>(this_->impl_)->getitervalue_(*var);
        }
        static const aug_itervtbl*
        vtbl()
        {
            static const aug_itervtbl local = {
                cast_,
                retain_,
                release_,
                islast_,
                next_,
                getkey_,
                getvalue_
            };
            return &local;
        }
    public:
        explicit
        iter(T* impl = 0)
        {
            this->iter_.vtbl_ = this->vtbl();
            this->iter_.impl_ = impl;
        }
        void
        reset(T* impl)
        {
            this->iter_.impl_ = impl;
        }
        aug_iter*
        get() AUG_NOTHROW
        {
            return &this->iter_;
        }
        operator aug::obref<aug_iter>() AUG_NOTHROW
        {
            return this->get();
        }
    };

    typedef aug::obref<aug_iter> iterref;
    typedef aug::smartob<aug_iter> iterptr;
}
#endif /* __cplusplus */

/* C++ definitions. */

#if defined(__cplusplus)
namespace aug {

    inline aug_bool
    isiterlast(aug::obref<aug_iter> this_) AUG_NOTHROW
    {
        return this_.get()->vtbl_->islast_(this_.get());
    }

    inline void
    nextiter(aug::obref<aug_iter> this_) AUG_NOTHROW
    {
        this_.get()->vtbl_->next_(this_.get());
    }

    inline void
    getiterkey(aug::obref<aug_iter> this_, struct aug_var& var)
    {
        verify(this_.get()->vtbl_->getkey_(this_.get(), &var), this_);
    }

    inline void
    getitervalue(aug::obref<aug_iter> this_, struct aug_var& var)
    {
        verify(this_.get()->vtbl_->getvalue_(this_.get(), &var), this_);
    }

    template <typename T>
    class iter_base {
        iter<T> iter_;
        int refs_;
    protected:
        ~iter_base()
        {
            /* Always deleted via derived, so no need to be virtual. */
        }
        iter_base()
            : refs_(1)
        {
            this->iter_.reset(static_cast<T*>(this));
        }
    public:
        aug::smartob<aug_object>
        cast_(const char* id) AUG_NOTHROW
        {
            if (aug::equalid<aug_object>(id) || aug::equalid<aug_iter>(id))
                return aug::object_retain<aug_object>(this->iter_);
            return null;
        }
        void
        retain_() AUG_NOTHROW
        {
            assert(0 < this->refs_);
            ++this->refs_;
        }
        void
        release_() AUG_NOTHROW
        {
            assert(0 < this->refs_);
            if (0 == --this->refs_)
                delete static_cast<T*>(this);
        }
        static aug::smartob<aug_iter>
        attach(T* ptr)
        {
            return aug::object_attach<aug_iter>(ptr->iter_);
        }
    };

    template <typename T>
    class scoped_iter_base {
        iter<T> iter_;
    protected:
        ~scoped_iter_base()
        {
        }
        scoped_iter_base()
        {
            this->iter_.reset(static_cast<T*>(this));
        }
    public:
        aug::smartob<aug_object>
        cast_(const char* id) AUG_NOTHROW
        {
            if (aug::equalid<aug_object>(id) || aug::equalid<aug_iter>(id))
                return aug::object_retain<aug_object>(this->iter_);
            return null;
        }
        void
        retain_() AUG_NOTHROW
        {
        }
        void
        release_() AUG_NOTHROW
        {
        }
        aug_object*
        base() AUG_NOTHROW
        {
            return aug::obref<aug_object>(this->iter_).get();
        }
        aug_iter*
        get() AUG_NOTHROW
        {
            return this->iter_.get();
        }
        operator aug::obref<aug_iter>() AUG_NOTHROW
        {
            return this->iter_;
        }
    };

    template <typename T>
    class iter_wrapper {
        iter<iter_wrapper<T> > iter_;
        T impl_;
        int refs_;
        explicit
        iter_wrapper(const T& impl)
            : impl_(impl),
              refs_(1)
        {
            this->iter_.reset(this);
        }
    public:
        aug::smartob<aug_object>
        cast_(const char* id) AUG_NOTHROW
        {
            if (aug::equalid<aug_object>(id) || aug::equalid<aug_iter>(id))
                return aug::object_retain<aug_object>(this->iter_);
            return null;
        }
        void
        retain_() AUG_NOTHROW
        {
            assert(0 < this->refs_);
            ++this->refs_;
        }
        void
        release_() AUG_NOTHROW
        {
            assert(0 < this->refs_);
            if (0 == --this->refs_)
                delete this;
        }
        aug_bool
        isiterlast_() AUG_NOTHROW
        {
            return this->impl_.isiterlast_();
        }
        void
        nextiter_() AUG_NOTHROW
        {
            this->impl_.nextiter_();
        }
        aug_result
        getiterkey_(struct aug_var& var) AUG_NOTHROW
        {
            return this->impl_.getiterkey_(var);
        }
        aug_result
        getitervalue_(struct aug_var& var) AUG_NOTHROW
        {
            return this->impl_.getitervalue_(var);
        }
        static aug::smartob<aug_iter>
        create(const T& impl = T())
        {
            iter_wrapper* ptr(new iter_wrapper(impl));
            return aug::object_attach<aug_iter>(ptr->iter_);
        }
    };

    template <typename T>
    class scoped_iter_wrapper {
        iter<scoped_iter_wrapper<T> > iter_;
        T impl_;
    public:
        explicit
        scoped_iter_wrapper(const T& impl = T())
            : impl_(impl)
        {
            this->iter_.reset(this);
        }
        aug::smartob<aug_object>
        cast_(const char* id) AUG_NOTHROW
        {
            if (aug::equalid<aug_object>(id) || aug::equalid<aug_iter>(id))
                return aug::object_retain<aug_object>(this->iter_);
            return null;
        }
        void
        retain_() AUG_NOTHROW
        {
        }
        void
        release_() AUG_NOTHROW
        {
        }
        aug_bool
        isiterlast_() AUG_NOTHROW
        {
            return this->impl_.isiterlast_();
        }
        void
        nextiter_() AUG_NOTHROW
        {
            this->impl_.nextiter_();
        }
        aug_result
        getiterkey_(struct aug_var& var) AUG_NOTHROW
        {
            return this->impl_.getiterkey_(var);
        }
        aug_result
        getitervalue_(struct aug_var& var) AUG_NOTHROW
        {
            return this->impl_.getitervalue_(var);
        }
        aug_object*
        base() AUG_NOTHROW
        {
            return aug::obref<aug_object>(this->iter_).get();
        }
        aug_iter*
        get() AUG_NOTHROW
        {
            return this->iter_.get();
        }
        operator aug::obref<aug_iter>() AUG_NOTHROW
        {
            return this->iter_;
        }
    };

    typedef aug::smartob<aug_iter> iterptr;
}
#endif /* __cplusplus */

#endif /* AUGEXT_ITER_H */
