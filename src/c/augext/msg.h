...
/* at Tue Jun 02 19:07:15 +0100 2009 */
...
namespace aug {
    ...
    class msg {
        ...
        static unsigned
        getid_(aug_msg* this_) AUG_NOTHROW
        {
            return static_cast<T*>(this_->impl_)->getmsgid_();
        }
        static const char*
...
namespace aug {
    ...
    inline unsigned
    getmsgid(aug::obref<aug_msg> this_) AUG_NOTHROW
    {
    ...
    protected:
        ...
        msg_base()
            : refs_(1)
        {
            this->msg_.reset(static_cast<T*>(this));
        }
    public:
    ...
    template <typename T>
    class msg_wrapper {
        msg<msg_wrapper<T> > msg_;
        T impl_;
        int refs_;
        explicit
        msg_wrapper(const T& impl)
            : impl_(impl),
              refs_(1)
        {
    ...
    public:
        ...
        {
            msg_wrapper* ptr(new msg_wrapper(impl));
            return aug::object_attach<aug_msg>(ptr->msg_);
        }
    };

    template <typename T>
    class scoped_msg_wrapper {
        msg<scoped_msg_wrapper<T> > msg_;
        T impl_;
    public:
        explicit
        scoped_msg_wrapper(const T& impl = T())
            : impl_(impl)
        {
            this->msg_.reset(this);
        }
