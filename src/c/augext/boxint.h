...
/* at Fri Jun 05 07:04:22 +0100 2009 */
...
struct aug_boxintvtbl {
    AUG_VTBL(aug_boxint);
    int (*unboxint_)(aug_boxint*);
};

#define aug_unboxint(this_) \
    (this_)->vtbl_->unboxint_(this_)
...
namespace aug {
    ...
    class boxint {
        ...
        static int
        unboxint_(aug_boxint* this_) AUG_NOTHROW
        {
            return static_cast<T*>(this_->impl_)->unboxint_();
        }
...
namespace aug {
    ...
    inline int
    unboxint(aug::obref<aug_boxint> this_) AUG_NOTHROW
    {
        return this_.get()->vtbl_->unboxint_(this_.get());
    }
    ...
    protected:
        ...
        boxint_base()
            : refs_(1)
        {
            this->boxint_.reset(static_cast<T*>(this));
        }
    public:
    ...
    template <typename T>
    class boxint_wrapper {
        boxint<boxint_wrapper<T> > boxint_;
        T impl_;
        int refs_;
        explicit
        boxint_wrapper(const T& impl)
            : impl_(impl),
              refs_(1)
        {
    ...
    public:
        ...
        int
        unboxint_() AUG_NOTHROW
        {
            return this->impl_.unboxint_();
        }
        static aug::smartob<aug_boxint>
        create(const T& impl = T())
        {
            boxint_wrapper* ptr(new boxint_wrapper(impl));
            return aug::object_attach<aug_boxint>(ptr->boxint_);
        }
    };

    template <typename T>
    class scoped_boxint_wrapper {
        boxint<scoped_boxint_wrapper<T> > boxint_;
        T impl_;
    public:
        explicit
        scoped_boxint_wrapper(const T& impl = T())
            : impl_(impl)
        {
            this->boxint_.reset(this);
        }
