...
/* at Fri Apr 09 19:04:41 +0100 2010 */
...
struct aug_httphandlervtbl {
    ...
    aug_result (*field_BIN_)(aug_httphandler*, const char*, const char*);
    aug_result (*csize_BIN_)(aug_httphandler*, unsigned);
    aug_result (*cdata_BIN_)(aug_httphandler*, const void*, unsigned);
...
#define aug_httpfield_BIN(this_, name, value) \
    (this_)->vtbl_->field_BIN_(this_, name, value)

#define aug_httpcsize_BIN(this_, size) \
    (this_)->vtbl_->csize_BIN_(this_, size)

#define aug_httpcdata_BIN(this_, buf, len) \
    (this_)->vtbl_->cdata_BIN_(this_, buf, len)
...
namespace aug {
    ...
    class httphandler {
        ...
        static aug_result
        field_BIN_(aug_httphandler* this_, const char* name, const char* value) AUG_NOTHROW
        {
            return static_cast<T*>(this_->impl_)->httpfield_BIN_(name, value);
        }
        static aug_result
        csize_BIN_(aug_httphandler* this_, unsigned size) AUG_NOTHROW
        {
            return static_cast<T*>(this_->impl_)->httpcsize_BIN_(size);
        }
        static aug_result
        cdata_BIN_(aug_httphandler* this_, const void* buf, unsigned len) AUG_NOTHROW
        {
            return static_cast<T*>(this_->impl_)->httpcdata_BIN_(buf, len);
        }
        static aug_result
...
namespace aug {
    ...
    inline aug_result
    httpfield_BIN(aug::obref<aug_httphandler> this_, const char* name, const char* value)
    {
        return verify(this_.get()->vtbl_->field_BIN_(this_.get(), name, value));
    }

    inline aug_result
    httpcsize_BIN(aug::obref<aug_httphandler> this_, unsigned size)
    {
        return verify(this_.get()->vtbl_->csize_BIN_(this_.get(), size));
    }

    inline aug_result
    httpcdata_BIN(aug::obref<aug_httphandler> this_, const void* buf, unsigned len)
    {
        return verify(this_.get()->vtbl_->cdata_BIN_(this_.get(), buf, len));
    }
    ...
    public:
        ...
        aug_result
        httpfield_BIN_(const char* name, const char* value) AUG_NOTHROW
        {
            return this->impl_.httpfield_BIN_(name, value);
        }
        aug_result
        httpcsize_BIN_(unsigned size) AUG_NOTHROW
        {
            return this->impl_.httpcsize_BIN_(size);
        }
        aug_result
        httpcdata_BIN_(const void* buf, unsigned len) AUG_NOTHROW
        {
            return this->impl_.httpcdata_BIN_(buf, len);
        }
        aug_result
