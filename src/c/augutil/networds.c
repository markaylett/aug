...
word_(struct words* st, int ch)
{
    if (st->flags_ & AUG_LEXNEWLINE) {
        ...
        case '\\': /* Escape. */
            /* Begin new word. */
            st->out_(st->arg_, AUG_TOKWORD);
            st->flags_ |= (AUG_LEXESCAPE | AUG_LEXLABEL);
            break;
        default:
            st->out_(st->arg_, AUG_TOKWORD);
            /* Append to word. */
    ...
    } else if (st->flags_ & AUG_LEXESCAPE) {
        /* Handle escape. */
        st->flags_ &= ~AUG_LEXESCAPE;
        /* Literal. */
        st->out_(st->arg_, ch);
